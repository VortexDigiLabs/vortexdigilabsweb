...
export function CardStreamScanner({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // --- Vanilla JS Logic Adapted for React ---
    const codeChars =
    ...
    class CardStreamController {
      ...
      clippingFrame: number | null = null;

      constructor(root: HTMLElement) {
        this.container = root.querySelector("#cardStream") as HTMLElement;
        this.cardLine = root.querySelector("#cardLine") as HTMLElement;
        this.speedIndicator = root.querySelector("#speedValue");
      ...
      setupEventListeners() {
        ...
        window.addEventListener("resize", this.calculateDimensionsBound);
      }

      onDragBound = (e: MouseEvent) => this.onDrag(e);
      endDragBound = () => this.endDrag();
      onTouchMoveBound = (e: TouchEvent) => this.onDrag(e.touches[0]);
      calculateDimensionsBound = () => this.calculateDimensions();

      startDrag(e: MouseEvent | Touch) {
        // e.preventDefault(); // Cannot prevent default on passive event listener if it's touch, but we set passive: false
      ...
      toggleAnimation() {
        ...
        const btn = container.querySelector(".control-btn-pause");
      ...
      createCardWrapper(index: number) {
        ...
        const cardImages = [
          ...
          "https://images.unsplash.com/photo-1510511459019-5efa3cae14dc?w=800&h=1200&fit=crop",
      ...
      updateCardClipping() {
        ...
        this.container.querySelectorAll(".card-wrapper").forEach((wrapper) => {
      ...
      updateAsciiContent() {
        this.container.querySelectorAll(".ascii-content").forEach((content) => {
          if (Math.random() < 0.15) {
      ...
      destroy() {
        ...
        window.removeEventListener("resize", this.calculateDimensionsBound);
    ...
    class ParticleSystem {
      ...
      constructor(root: HTMLElement) {
        this.canvas = root.querySelector("#particleCanvas") as HTMLCanvasElement;
      ...
      onWindowResizeBound = () => this.onWindowResize();
    ...
    class ParticleScanner {
      ...
      constructor(root: HTMLElement) {
        this.canvas = root.querySelector("#scannerCanvas") as HTMLCanvasElement;
    ...
    const cardStream = new CardStreamController(container);
    const particleSystem = new ParticleSystem(container);
    const particleScanner = new ParticleScanner(container);
    ...
    const toggleBtn = container.querySelector(".control-btn-pause");
    const resetBtn = container.querySelector(".control-btn-reset");
    const dirBtn = container.querySelector(".control-btn-dir");

    const onToggle = () => cardStream.toggleAnimation();
    const onReset = () => cardStream.resetPosition();
    const onDir = () => cardStream.changeDirection();
  ...
  return (
    <div className={cn("relative w-full h-full min-h-screen bg-black overflow-hidden font-sans", className)} ref={containerRef}>
      ...
      <div className="controls">
        <button className="control-btn control-btn-pause">⏸️ Pause</button>
        <button className="control-btn control-btn-reset">🔄 Reset</button>
        <button className="control-btn control-btn-dir">↔️ Direction</button>
      </div>
